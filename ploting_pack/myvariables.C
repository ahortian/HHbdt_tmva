...
void myvariables( TString fin = "TMVA.root", TString dirout = "plots", TString dirName = "InputVariables_Id", TString title = "TMVA Input Variables",
                Bool_t isRegression = kFALSE, Bool_t useTMVAStyle = kTRUE )
{
   ...
   while ((key = (TKey*)next())) {
      ...
      if (countPad%noPadPerCanv==0) {
         ...
         TString fname = dirout + Form( "/%s_c%i", outfname.Data(), countCanvas );
