...
void plot_efficiencies( TFile* file, TString dirout = "plots", Int_t type = 2, TDirectory* BinDir=0)
{
...
void myefficiencies( TString fin = "TMVA.root", TString dirout = "plots", Int_t type = 2, Bool_t useTMVAStyle = kTRUE )
{
   ...
   while ((key = (TKey*)nextDir())) {
      ...
      if (path.Contains("multicutMVA")){
         multiMVA=kTRUE;
         plot_efficiencies( file, dirout, type, d );
      }
   }
   plot_efficiencies( file, dirout, type, gDirectory );
