...
void mymvas( TString fin = "TMVA.root", TString dirout = "plots", HistType htype = CompareType, Bool_t useTMVAStyle = kTRUE )
{
   ...
   while ((key = (TKey*)next())) {
	 ...
	 std::cout << "--- Found directory for method: " << methodName << "::" << methodTitle << std::flush;
		 ...
		 //if ( htype == CompareType && TString(key->GetName()).BeginsWith("Method_Likelihood") ) {
            ...
            cout << "--- Perform Kolmogorov-Smirnov tests" << endl;
			 TH1* sig_tmp = (TH1*) sig->Clone();
			 TH1* bgd_tmp = (TH1*) bgd->Clone();
			 TH1* sigOv_tmp = (TH1*) sigOv->Clone();
			 TH1* bgdOv_tmp = (TH1*) bgdOv->Clone();
			 
			 for (int i=1; i<=int(sig_tmp->GetNbinsX()); i++){
				 if (sig_tmp->GetBinContent(i) < 0.){
					 cout << " sig_tmp test BinContent < 0 : bin " << i << " : " << sig_tmp->GetBinContent(i) << endl;
					 sig_tmp->SetBinContent(i, 0.);
					 sig_tmp->SetBinError(i, 0.);
				 }
				 if (sigOv_tmp->GetBinContent(i) < 0.){
					 cout << " sigOv_tmp train BinContent < 0 : bin " << i << " : " << sigOv_tmp->GetBinContent(i) << endl;
					 sigOv_tmp->SetBinContent(i, 0.);
					 sigOv_tmp->SetBinError(i, 0.);
				 }
			 }
			 for (int i=1; i<=int(bgd_tmp->GetNbinsX()); i++){
				 if (bgd_tmp->GetBinContent(i) < 0.){
					 cout << " bgd_tmp test BinContent < 0 : bin " << i << " : " << bgd_tmp->GetBinContent(i) << endl;
					 bgd_tmp->SetBinContent(i, 0.);
					 bgd_tmp->SetBinError(i, 0.);
				 }
				 if (bgdOv_tmp->GetBinContent(i) < 0.){
					 cout << " bgdOv_tmp train BinContent < 0 : bin " << i << " : " << bgdOv_tmp->GetBinContent(i) << endl;
					 bgdOv_tmp->SetBinContent(i, 0.);
					 bgdOv_tmp->SetBinError(i, 0.);
				 }
			 }
			 Double_t kolS = sig_tmp->KolmogorovTest( sigOv_tmp, "X" );
			 Double_t kolB = bgd_tmp->KolmogorovTest( bgdOv_tmp, "X" );
         ...
         if (Save_Images) {
            if      (htype == MVAType)     TMVAGlob::imgconv( c, dirout + Form("/mva_%s",     methodTitle.Data()) );
            else if (htype == ProbaType)   TMVAGlob::imgconv( c, dirout + Form("/proba_%s",   methodTitle.Data()) );
            else if (htype == CompareType) TMVAGlob::imgconv( c, dirout + Form("/overtrain_%s", methodTitle.Data()) );
            else                           TMVAGlob::imgconv( c, dirout + Form("/rarity_%s",  methodTitle.Data()) );
         }
